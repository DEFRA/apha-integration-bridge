...
IS NULL` only) but is verified in-container.
    **D6 refinement (adopted during implementation):** the surrogate PKs
    (`*_STATE_PK`, `PARTY_VERSION_PK`, `ADDRESS_USAGE_PK`, `FEATURE_ADDRESS_PK`,
    `FEATURE_POINT_PK`, `ASSET_LOCATION_PK`, `FEATURE_INVOLVEMENT_PK`) are populated
    by `GENERATED BY DEFAULT AS IDENTITY` — the exact pattern the original fixture
    already used for `FEATURE_INVOLVEMENT_PK`. This auto-fills the surrogate on every
    MERGE-WHEN-NOT-MATCHED-INSERT and plain INSERT branch, so the ~30 existing state/
    version/usage producers need **no per-row edits**; the column is a NOT NULL NUMBER,
    query-neutral, and one-open-row-per-entity is still guaranteed by the existing
    MERGE-on-entity-pk / single-INSERT logic. New non-surrogate NOT NULL columns
    continue to use literal `DEFAULT`s (the Stage-2 pattern). This is strictly safer
    and less error-prone than threading entity-pk-derived literals through every
    producer; the surrogate values are not deterministic across schemas but are never
    read by any query.
...
- **Stage 3 (Gemini HIGH / Codex MEDIUM) — "IDENTITY surrogate PKs drop natural-key
  uniqueness on ADDRESS_USAGE / PARTY_VERSION; add non-canonical UNIQUE indexes to
  guard against fan-out."** _Recommendation rejected._ The canonical SAM schema
  deliberately has **no** natural-key UK on these tables (only the surrogate PK); a
  PARTY_CONTACT_ADDRESS may legitimately carry multiple ADDRESS_USAGE rows and a
  PARTY multiple PARTY_VERSION rows (history), so a guard UK on `(pca_pk,
usage_type)` / `(party_pk, party_type)` would both deviate from canonical and risk
  rejecting valid multi-row data. The seed is verified single-open-row-per-entity
  (all three reviewers + the container diff confirm no fan-out today). Resolved by a
  documented HAZARD comment at the ADDRESS_USAGE definition instead of a UK, so
  future seed authors don't introduce a second open row. (opencode agreed this is
  "not a fixture defect — faithfully reproduces canonical".)
