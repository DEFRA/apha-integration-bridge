...
class SalesforceClient {
  ...
  async getKeyFacts(applicationId, logger) {
    const token = await this.getAccessToken(logger)
    const query = `SELECT ID, APHA_Key__c, APHA_Value__c, APHA_EntityType__c, APHA_Status__c, APHA_ObjectAPIName__c, APHA_RecordId__c FROM APHA_KeyFact__c WHERE APHA_Application__c='${applicationId}'`
    return this.sendQuery(query, token, logger)
  }
