...
describe('HTTPArrayResponse', () => {
  ...
  test('array response throws when an item includes links', () => {
    const item = new HTTPObjectResponse('user', 1, {
      username: 'alice'
    }).links({ self: '/users/1' })
    const list = new HTTPArrayResponse().add(item)
  ...
  test('add() and remove() are chainable', () => {
    ...
    expect(list.add(new HTTPObjectResponse('x', 'a', {}))).toBe(list)
    expect(
      list
        .add(new HTTPObjectResponse('x', 'b', {}))
        .remove('a')
    ).toBe(list)
    expect(list.toResponse()).toEqual({
      data: [{ type: 'x', id: 'b' }]
