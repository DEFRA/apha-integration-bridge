...
(
  ...
  wsl.entityid,
  wsl.pyid,
  pv.party_type customer_type

  FROM
  pega_Data.index_ac_wsentities wsl,
  ahbrp.party p,
  ahbrp.party_version pv

  WHERE
  wsl.PXINDEXPURPOSE = 'workScheduleCustomers'
  AND
  wsl.entityid = p.party_id(+)
  AND
  p.party_pk = pv.party_pk(+)
  AND
  pv.party_version_to_datetime(+) = '31-DEC-9999'
) ws_c,
(
  SELECT
  entityid,
