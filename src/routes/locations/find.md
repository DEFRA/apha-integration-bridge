...
- The response contains only the locations that were successfully found and are active

### Data Ordering

- Results are returned in the same order as the IDs in the request
- This allows clients to maintain consistent ordering when processing results
...
Response: All 3 locations (if found and active)
...
```bash
POST /locations/find?page=1&pageSize=2
{
  "ids": ["L1", "L2", "L3", "L4", "L5"]
}
```

Response: Returns L1 and L2 with `links.next` pointing to page 2

Request:

```bash
POST /locations/find?page=2&pageSize=2
{
  "ids": ["L1", "L2", "L3", "L4", "L5"]
}
```

Response: Returns L3 and L4 with `links.next` pointing to page 3 and `links.prev` to page 1

### Example 3: Missing Locations
...
If L999 doesn't exist, response will contain only L123 and L456 (assuming they exist and are active).
