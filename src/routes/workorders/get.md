...
- Filters by one or more `status` values; returns only `Open` work orders when omitted.
...
| `status`   | string or array | No       | `Open`  | Filters work orders by status. Accepts `New`, `Open`, `Open-Reopened`, `Pending-Activated`, `Pending-Planned`, `Resolved-Cancelled`, `Resolved-Closed`, `Resolved-Completed`, or `Resolved-Not-Required` (case-insensitive). Use multiple query parameters for OR logic (e.g., `status=Open&status=Resolved-Closed`). When omitted, returns only `Open` work orders. |
...
**Filtering by status (returns only `Open` work orders when omitted):**

```bash
curl -X GET \
  'https://<host>/workorders?startActivationDate=2024-01-01T00:00:00.000Z&endActivationDate=2024-02-01T00:00:00.000Z&status=Open&status=Resolved-Closed&page=1&pageSize=10' \
  -H 'Authorization: Bearer <token>' \
  -H 'Accept: application/vnd.apha.1+json'
```
...
- Leaving `status` out is not the same as asking for everything: the endpoint falls back to `Open`. Pass the statuses you want (e.g., `status=Open&status=Resolved-Closed`) to widen the results.
